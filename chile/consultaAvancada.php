...
if(isset( $_POST['consultar'])){
     if ($_POST['pesquisa2']){        
        ...
        $dataFinal = $_POST['dataFinalAno']."-".$_POST['dataFinalMes']."-31";
        $sqlVerificaNcm = "Select id from ".$cap." where DATAMOV BETWEEN '".$dataInicial."' AND '".$dataFinal."' and ncm like '".$_POST['pesquisa2']."%' limit 1";
    ...
    $_SESSION["dataFinal"] = $_POST['dataFinalAno']."-".$_POST['dataFinalMes']."-31";
    $_SESSION["valorUnitarioIni"] = $_POST['valorUnitarioIni'];
    $_SESSION["valorUnitarioFin"] = $_POST['valorUnitarioFin'];
    $_SESSION["totalpagina"] = 0;
    
    $sqlLog = "insert into log (DATACONSULTA, NCM, DESCRICAO, DATAINICIAL, DATAFINAL, MODULO,IDUSUARIO) VALUES (
                '".date("Y/m/d H:i:s")."',
                '".$_SESSION["ncm"]."',
                '".$_SESSION["descricao1"]."',
...
<form action="consultaAvancada.php?pagina=1" method="POST">
<table align=center>
    ...
    <tr>
        <td> Modelo</td>
        <td><input type="text" name="modelo" value="<?=$_SESSION["modelo"]?>" size="70"/></td>
    </tr>
    <tr>
        <td> NCM </td>
...
$sql = geraSQLConsultaAvancada($_SESSION["descricao1"],$_SESSION["modelo"],$_SESSION["ncm"],$_SESSION["valorUnitarioIni"],$_SESSION["valorUnitarioFin"],$_SESSION["dataInicial"],$_SESSION["dataFinal"],$tampag,$qtd,"");
...
<thead>
    ...
    <tr class="<?=$class?>">
        ...
        <td style="width: 80; text-align: left;"> <?=$dados['IMPORTADORESDOSETOR1']?></td>
        <td style="width: 80; text-align: left;"> <?=$dados['MARCA']?></td>
        <td style="width: 80; text-align: left;"> <?=$dados['MODELO']?></td>
        <td style="width: 80; text-align: left;"> <?=$dados['ATRIBUTO']?></td>
        ...
        <td style="width: 80; text-align: right;"> <?=number_format($dados['FRETE'],2,",",".")?></td>
        <td style="width: 80; text-align: right;"> <?=number_format($dados['SEGURO'],2,",",".")?></td>
        <td style="width: 80; text-align: right;"> <?=number_format($dados['CIF'],2,",",".")?></td>
...
<center>
    ...
    <form name="consulta" action="consultaAvancada.php?pagina=n" method="POST">
    <?php
    if($pagina >1){?>
        <a href="consultaAvancada.php?pagina=<?=$pagina-1?>"> página anterior</a> &nbsp;
    <?php };?>
        ...
        if($pagina < $_SESSION["totalpagina"]){?>
         &nbsp; <a href="consultaAvancada.php?pagina=<?=$pagina+1?>"> próxima página </a>
         <?php }};?>
    </form>
